...
import { Search, Layers, ChevronRight, ShoppingCart } from 'lucide-react';

const GiftCardsView: React.FC = () => {
  return (
    <div className="space-y-16 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {['STEAM', 'PLAYSTATION', 'XBOX', 'NINTENDO', 'APPLE'].map(cat => (
            <button key={cat} className="bg-white/5 border border-white/5 px-6 py-2.5 rounded-full text-[10px] font-black uppercase hover:bg-white/10 transition-all tracking-widest">{cat}</button>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {GIFT_CARDS.map(card => (
          <div key={card.id} className="bg-[#161a23]/60 border border-white/5 rounded-[2rem] p-8 flex items-center justify-between hover:border-[#7e9dff]/30 group transition-all">
            <div className="flex items-center space-x-6">
              <div className="w-14 h-14 bg-black/40 rounded-2xl p-3 flex items-center justify-center group-hover:bg-black transition-colors">
                <img src={card.image} className="w-full h-full object-contain" alt={card.brand} />
              </div>
      ...
      <div className="bg-[#161a23]/30 border border-white/5 p-16 rounded-[3rem] text-center flex flex-col items-center">
        ...
        <button className="bg-yellow-500 hover:bg-yellow-600 text-[#0b0e14] px-16 py-6 rounded-2xl font-black text-xl uppercase shadow-[0_10px_40px_rgba(234,179,8,0.2)] transition-all active:scale-95 tracking-tighter">
          СВЯЗАТЬСЯ С НАМИ
        </button>
      </div>
    </div>
